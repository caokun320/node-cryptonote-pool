    ...
    async.waterfall([
        ...
        function(blocks, callback){
            async.filter(blocks, function(block, mapCback){
                apiInterfaces.rpcDaemon('getblockhash', [block.height], function(error, result){
                    if (error){
                        log('error', logSystem, 'Error with getblockhash RPC request for block %s - %j', [block.serialized, error]);
                        block.unlocked = false;
                        mapCback();
                        return;
                    }
                    let blockhash = result;
                    apiInterfaces.rpcDaemon('getblockheader', [blockhash], function(error, result){
                        if (error){
                            log('error', logSystem, 'Error with getblockheader RPC request for block %s - %j', [block.serialized, error]);
                            block.unlocked = false;
                            mapCback();
                            return;
                        }
                        if (!result){
                            log('error', logSystem, 'Error with getblockheader, no details returned for %s - %j', [block.serialized, result]);
                            block.unlocked = false;
                            mapCback();
                            return;
                        }
                        var blockHeader = result;
