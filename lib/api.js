...
function collectStats(){
    ...
    async.parallel({
        ...
        network: function(callback){
            apiInterfaces.rpcDaemon('getbestblockhash', [], function(error, bestBlockHash){
                if (error){
                    log('error', logSystem, 'Error getting daemon data getbestblockhash %j', [error]);
                    callback(true);
                    return;
                }
                apiInterfaces.rpcDaemon('getblockheader', [bestBlockHash], function(error, blockHeader){
