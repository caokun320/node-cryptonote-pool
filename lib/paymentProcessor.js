    ...
    async.waterfall([
        ...
        function(balances, callback){
            ...
            for (var worker in payments){
                ...
                console.log(`JWU amount: ${amount} for address: ${worker}`);
                transferCommands[commandIndex].rpc[worker] = amount;
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                let sendAmount = JSON.stringify(transferCmd.rpc);
                console.log('!!!!! JWU start sendmany: ' + sendAmount);
                apiInterfaces.rpcWallet('sendmany', ["", transferCmd.rpc], function(error, result){
                    if (error){
                        log('error', logSystem, 'Error with transfer RPC request to wallet daemon %j', [error]);
                        log('error', logSystem, 'Payments failed to send to %j', [transferCmd.rpc]);
                        cback(false);
                        return;
                    }

                    var now = (timeOffset++) + Date.now() / 1000 | 0;
                    console.log('JWU sendmany result is:')
                    console.log(result)
