...
function BlockTemplate(template){
    var hashNum = bignum.fromBuffer(Buffer.from(template.target, 'hex'));
    this.difficulty = diff1.div(hashNum);
    this.height = template.height;
    this.reward = template.coinbasevalue;
    this.extraNonce = 0;
    this.extraNoncePlaceholder = Buffer.from('f000000ff111111f', 'hex');
    this.poolAddressScript = utils.addressToScript(config.poolServer.poolAddress);
    this.template = template;
    this.nTime = template.curtime;
    this.jobTemplate = new blockTemplate(
        0, // jobId
        this.template,
...
BlockTemplate.prototype = {
    convertToCNHeader: function(originalHeader){
        // Copy the first 39 bytes.
        var current = originalHeader.slice(0, 39);
        // Concat 4 bytes nonce.
        current = Buffer.concat([current, originalHeader.slice(76, 80)], 43);
        // Concat remaining 29 bytes of merkle root.
        current = Buffer.concat([current, originalHeader.slice(39, 39 + 29)], 72);
        // Concat the timestamp and bits.
        current = Buffer.concat([current, originalHeader.slice(68, 76)], 80);
        return current;
    },

    generateBlobs: function(job, extraNonce, nonceBuffer){
        var extraNonce1Buffer = instanceId;
        var extraNonce2Buffer = Buffer.alloc(4);
        extraNonce2Buffer.writeUInt32BE(extraNonce, 0);

        var coinbaseBuffer = job.serializeCoinbase(extraNonce1Buffer, extraNonce2Buffer);
        var coinbaseHash = utils.sha256d(coinbaseBuffer);
        var merkleRoot = utils.reverseBuffer(job.merkleTree.withFirst(coinbaseHash)).toString('hex');
        var nTimeBuffer = Buffer.alloc(4);
        nTimeBuffer.writeUInt32BE(this.nTime, 0);
        var reversedNonce = utils.reverseBuffer(nonceBuffer).toString('hex');
        var originalHeader = job.serializeHeader(merkleRoot, nTimeBuffer.toString('hex'), reversedNonce);
        return {
            coinbase: coinbaseBuffer,
            coinbaseHash: coinbaseHash,
            header: originalHeader,
            cnheader: this.convertToCNHeader(originalHeader)
        };
    },
    ...
    nextBlob: function(){
        ...
        return this.generateBlobs(this.jobTemplate, this.extraNonce, nonceBuffer).cnheader;
    }
};

function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('getblocktemplate', {}, callback);
}
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height){
            processBlockTemplate(result.height, result);
        }
        callback(true);
    })
}

function processBlockTemplate(height, template){
    ...
    log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d', [height, currentBlockTemplate.difficulty]);
...
(function init(){
    apiInterfaces.rpcWallet('validateaddress', [config.poolServer.poolAddress], function(error, result) {
        if (error) {
            log('error', logSystem, 'Failed to rpc call validateaddress.')
            process.exit(1);
        }
        if (!result.ismine) {
            log('error', logSystem, 'Wallet does not own pool address %s - payment processing can not be done with this daemon.', [config.poolServer.poolAddress])
            process.exit(1);
        }
...
Miner.prototype = {
    ...
    getJob: function(){
        ...
        var blob = currentBlockTemplate.nextBlob().toString('hex');
...
function recordShareData(miner, job, shareDiff, blockCandidate, coinbaseTxId, shareType, blockTemplate){
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            redisClient.hgetall(config.coin + ':shares:roundCurrent', function(err, results){
                if (err) {
                    log('error', logSystem, 'Failed to hgetall items from current round %j \n %j', [err, redisCommands]);
                    return;
                }
                let counter = 0;
                for (let i = 0; i < results.length - 1; i = i + 2) {
                    let key = config.coin + ':shares:round' + job.height;
                    redisClient.hincrby(key, results[i], results[i+1], function(err, result) {
                        counter ++;
                        if (counter == result.length/2) {
            ...
            redisClient.zadd(config.coin + ':blocks:candidates', job.height, [
                ...
                if (err){
                    log('error', logSystem, 'Failed inserting block candidate coinbase tx %s \n %j', [coinbaseTxId, err]);
                }
            });
        }
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        // TODO: test this. DO NOT use it for now.
        hash = Buffer.from(resultHash, 'hex');
        shareType = 'trusted';
    } else {
        let blobs = blockTemplate.generateBlobs(blockTemplate.jobTemplate, job.extraNonce, Buffer.from(nonce, 'hex'));
        headerBlob = blobs.header;
        cnHeaderBlob = blobs.cnheader;
        coinbase = blobs.coinbase;
        const cn_variant = 2;
        hash = cryptoNight(cnHeaderBlob, cn_variant);
        shareType = 'valid';
    }
    ...
    let hashArray = hash.toJSON().data;
    hashArray.reverse();
    let hashNum = bignum.fromBuffer(Buffer.from(hashArray));
    let hashDiff = diff1.div(hashNum);

    if (hashDiff.ge(blockTemplate.difficulty)){
        let blockHash = utils.reverseBuffer(utils.sha256d(headerBlob)).toString('hex');
        let shareBuffer = blockTemplate.jobTemplate.serializeBlock(headerBlob, coinbase);
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('info', logSystem,
                    'Blockfound at height %d by miner %s@%s blockhash %s - submit result: %j',
                    [job.height, miner.login, miner.ip, blockHash.substr(0, 6), result]
                );
                apiInterfaces.rpcDaemon('getblock', [blockHash], function(error, result){
                    if (error){
                        log('error', logSystem, 'Error getblock with blockHash: %s - %j', [blockHash, error]);
                        recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
                    } else {
                        let coinbaseTx = result.tx[0];
                        recordShareData(miner, job, hashDiff.toString(), true, coinbaseTx, shareType, blockTemplate);
                        jobRefresh();
                    }
                })
            }
