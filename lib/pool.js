...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        coinbaseHash = blobs.coinbaseHash;
        const cn_variant = 2;
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('info', logSystem,
                    'Blockfound at height %d by miner %s@%s coinbase txid %s - submit result: %j',
                    [job.height, miner.login, miner.ip, coinbaseHash.substr(0, 6), result]
                );
                recordShareData(miner, job, hashDiff.toString(), true, coinbaseHash, shareType, blockTemplate);
                jobRefresh();
            }
        });
