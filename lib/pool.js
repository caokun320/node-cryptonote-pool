...
BlockTemplate.prototype = {
    convertToCNHeader: function(originalHeader){
        // Combine nVersion and prevblockhash into 32 byte buffer.
        var current = utils.sha256(originalHeader.slice(0, 36));
        // Concat 7 bytes of merkle root.
        current = Buffer.concat([current, originalHeader.slice(36, 36 + 7)], 39);
        // Concat 4 bytes nonce.
        current = Buffer.concat([current, originalHeader.slice(76, 80)], 43);
        // Concat the rest of merkle root.
        current = Buffer.concat([current, originalHeader.slice(36 + 7, 36 + 32)], 68);
        // Concat timestamp and bits.
        current = Buffer.concat([current, originalHeader.slice(68, 68 + 8)], 76);
        return current;
    },
    ...
    generateBlobs: function(job, extraNonce, nonceBuffer){
        ...
        extraNonce2Buffer.writeUInt32LE(extraNonce, 0);
        ...
        nTimeBuffer.writeUInt32LE(this.nTime, 0);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                //var blockFastHash = cryptoNightFast(cnHeaderBlob || cnUtil.convert_blob(shareBuffer)).toString('hex');
                var blockFastHash = utils.sha256(shareBuffer).toString('hex');
                log('info', logSystem,
